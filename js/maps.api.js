...
document.getElementById('send_message').addEventListener('click', function(event) {
  ...
  // API URL: Verilən link
  const apiUrl =  'https://ebim-mtk.azurewebsites.net/' ;  // API URL-ini doğru daxil edin

  // Form məlumatları (burada nümunə olaraq sadə məlumatlar istifadə edirik)
  const formData = {
      Name: 'John Doe',
      PhoneNumber: '+994773698555',
  ...
  fetch(apiUrl, {
      ...
      body: JSON.stringify(formData)  // JSON formatında məlumat göndəririk
  })
  .then(response => response.json())  // Server cavabını JSON formatında alırıq
  .then(data => {
    console.log(data,"data");
    
      // Cavaba əsaslanaraq müvafiq mesajı göstəririk
      if (data.success) {  // Server uğurlu cavab verərsə
          document.getElementById('mail_success').style.display = 'block'; // Uğurlu mesajı göstər
          document.getElementById('mail_fail').style.display = 'none'; // Xəta mesajını gizlət
      } else {  // Serverdan uğursuz cavab alındıqda
          document.getElementById('mail_success').style.display = 'none'; // Uğurlu mesajı gizlət
          document.getElementById('mail_fail').style.display = 'block'; // Xəta mesajını göstər
      }
  })
  .catch(error => {
      // Hər hansı bir xəta baş verərsə (şəbəkə və ya server problemi)
      document.getElementById('mail_success').style.display = 'none'; // Uğurlu mesajı gizlət
      document.getElementById('mail_fail').style.display = 'block'; // Xəta mesajını göstər
...
fetch('https://ebim-mtk.azurewebsites.net/complexes/get')
